...
log.warn = function() {
  ...
  if (arguments.length > 0) {
    writeln('>> '[grunt.config('log.warn') || 'red'] + grunt.util._.trim(msg).replace(/\n/g, '\n>> '[grunt.config('log.warn') || 'red']));
  } else {
    writeln('ERROR'[grunt.config('log.warn') || 'red']);
  }
  return log;
};
log.error = function() {
...
log.ok = function() {
  var msg = format(arguments);
  if (arguments.length > 0) {
    writeln('>> '[grunt.config('log.ok') || 'green'] + grunt.util._.trim(msg).replace(/\n/g, '\n>> '[grunt.config('log.ok') || 'green']));
  } else {
    writeln('OK'[grunt.config('log.ok') || 'green']);
  }
  return log;
};
...
log.success = function() {
  var msg = format(arguments);
  writeln(msg[grunt.config('log.success') || 'green']);
  return log;
};
log.fail = function() {
  var msg = format(arguments);
  writeln(msg[grunt.config('log.fail') || 'red']);
  return log;
};
log.header = function() {
  var msg = format(arguments);
...
log.debug = function() {
  ...
  if (grunt.option('debug')) {
    writeln('[D] ' + msg[grunt.config('log.debug') || 'magenta']);
  }
  return log;
};
